function varargout = work3_2(varargin)
...
% work3_2 MATLAB code for work3_2.fig
%      work3_2, by itself, creates a new work3_2 or raises the existing
%      singleton*.
%
%      H = work3_2 returns the handle to a new work3_2 or the handle to
%      the existing singleton*.
%
%      work3_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in work3_2.M with the given input arguments.
%
%      work3_2('Property','Value',...) creates
%a new work3_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before work3_2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to work3_2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help work3_2

% Last Modified by GUIDE v2.5 19-Jun-2017 16:03:23
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @work3_2_OpeningFcn, ...
                   'gui_OutputFcn',  @work3_2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before work3_2 is made visible.
function work3_2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to work3_2 (see VARARGIN)

% Choose default command line output for work3_2
...
%uiwait(handles.gzggui); 
handles.Izvalue=Izvalue;
guidata(hObject, handles);

% UIWAIT makes work3_2 wait for user response (see UIRESUME)
% uiwait(handles.work32);


% --- Outputs from this function are returned to the command line.
function varargout = work3_2_OutputFcn(hObject, eventdata, handles) 
...
if isnan(Ixc_rectangle)
    Ixc_rectangle=[0];
end
if isnan(Ixc_circle)
    Ixc_circle=[0];
end
if isnan(Ixc_circlering)
    Ixc_circlering=[0];
end
...
Iz=Ix-Sall.*(Yc.^2);
set(handles.XX,'String',num2str(Iz));
elseif flag==1
    set(handles.XX,'String',num2str(Iz));
end
save Iz2 Iz
...
if (~((0<=x1)&&(x1<=1000)&&(0<=y1)&&(y1<=1000)&&(0<=x2)&&(x2<=1000)&&(0<=y2)&&(y2<=1000)&&(0<=x3)&&(x3<=1000)&&(0<=y3)&&(y3<=1000)&&((a+b)>c)&&((a+c)>b)&&((c+b)>a)))||(isnan(x1))||(isnan(y1))||(isnan(x2))||(isnan(y2))||(isnan(x3))||(isnan(y3))
    ...
    if a >0                                                                %确保不是空矩阵
        for jj =1:aa
            patch(xloc(jj,:),yloc(jj,:),[0.5 0.5 0.5])                      %画三角形
        end
    end
end
set(gca,'XLim',[0 1000]);
...
% % handles    structure with handles and user data (see GUIDATA)
% [out1,Izvalue]=gzg2;
% set(handles.edit7,'string',num2str(Izvalue));
gzg2
...
% --- Executes during object creation, after setting all properties.
function work32_CreateFcn(hObject, eventdata, handles)
% hObject    handle to work32 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns
% called
% axes(handles.work_3back)
% imshow('水立方.png');
...
% --- Executes during object creation, after setting all properties.
function work3_2_2back_CreateFcn(hObject, eventdata, handles)
% hObject    handle to work3_2_2back (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
imshow('水立方.png');
% Hint: place code in OpeningFcn to populate work3_2_2back


% --- Executes during object creation, after setting all properties.
function work_3back_CreateFcn(hObject, eventdata, handles)
% hObject    handle to work_3back (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
imshow('水立方.png');
% Hint: place code in OpeningFcn to populate work_3back
